...
// Redux Actions
import { updateBasket, UpdateBasketAction } from '../../lib/redux/actions/basketActions';
import { addProductsToStore, AddProductsToStoreAction } from '../../lib/redux/actions/productActions';

// Functions
import { rangeFormat, hasDatePassed } from '../../lib/helpers';
import { viewItemGaEvent, addToBasketGaEvent } from '../../lib/gaEcommerce';
...
function Product(props: Props): ReactElement {
  ...
  const [basketPayload, setBasketPayload] = useState({
    quantity: '1',
    deliveryType: product.deliveryMethods.length > 0 ? product.deliveryMethods[0] : 'collection',
    deliveryDate: ''
  });
    ...
    <>
      ...
      <form id="update-basket">
        {(product.deliveryMethods.includes('collection') || product.deliveryMethods.includes('delivery')) && (
          <>
            {product.deliveryMethods.includes('collection') && (
              <RadioInline
                id="deliveryType"
                label="Collection"
                name="collection"
...
function mapStateToProps(
  state: AppState, ownProps: RouteComponentProps<RouterParams>
) {
